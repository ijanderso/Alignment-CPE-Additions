...
class Alignment:
    def __init__(self, name, config, runGeomComp = "1"):
        ...
        config.checkInput(section,
                          knownSimpleOptions = ['globaltag', 'style', 'color', 'TemplateAddition'],
        ...
        self.templateAddition = "NotSet"
        if (config.exists(section,"TemplateAddition")): self.templateAddition = config.get(section,"TemplateAddition")
        ...
        for option in theConfig.options( theSection ):
            if option.startswith( "condition " ):
                ...
                if len(condPars) == 1:
                    ...
                    elif self.__shorthandExists(rcdName, condPars[0]) and rcdName != "TemplateAddition":
    ...
    def getRepMap( self ):
        if self.templateAddition != "NotSet":
            replacetext = str(self.templateAddition)
            temp = configTemplates.TemplateAddition.replace(".oO[STID]Oo.",replacetext)
        else:
            temp = ""
        result = {
            "name": self.name,
