//
//  SiStripTemplate.cc (v2.11) [v1.00 based on SiPixelTemplate v8.20)
//
...
//  V2.11 - add correction factor: measured/true charge
//  V2.12 - protect against negative error^2
//
...
// next, loop over all y-angle entries   
		...
		for (i=0; i < theCurrentTemp.head.NTy; ++i) {     
			...
			>> theCurrentTemp.enty[i].mpvvav2 >> theCurrentTemp.enty[i].sigmavav2 >> theCurrentTemp.enty[i].kappavav2 >> theCurrentTemp.enty[i].r_qMeas_qTrue;
			...
			>> theCurrentTemp.enty[i].spare[0] >> theCurrentTemp.enty[i].spare[1] >> theCurrentTemp.enty[i].spare[2] >> theCurrentTemp.enty[i].spare[3]
			>> theCurrentTemp.enty[i].spare[4] >> theCurrentTemp.enty[i].spare[5];
		...
		for (k=0; k < theCurrentTemp.head.NTyx; ++k) { 
			...
			for (i=0; i < theCurrentTemp.head.NTxx; ++i) { 
				...
				>> theCurrentTemp.entx[k][i].mpvvav2 >> theCurrentTemp.entx[k][i].sigmavav2 >> theCurrentTemp.entx[k][i].kappavav2 >> theCurrentTemp.entx[k][i].r_qMeas_qTrue;
				...
				>> theCurrentTemp.entx[k][i].spare[0] >> theCurrentTemp.entx[k][i].spare[1] >> theCurrentTemp.entx[k][i].spare[2] >> theCurrentTemp.entx[k][i].spare[3]
				>> theCurrentTemp.entx[k][i].spare[4] >> theCurrentTemp.entx[k][i].spare[5];
...
// sxmax is the maximum allowed strip charge (used for truncation)
	...
	r_qMeas_qTrue_=(1.f - yxratio)*((1.f - xxratio)*theStripTemp_[index_id_].entx[iylow][ilow].r_qMeas_qTrue + xxratio*theStripTemp_[index_id_].entx[iylow][ihigh].r_qMeas_qTrue)
                   +yxratio*((1.f - xxratio)*theStripTemp_[index_id_].entx[iyhigh][ilow].r_qMeas_qTrue + xxratio*theStripTemp_[index_id_].entx[iyhigh][ihigh].r_qMeas_qTrue);
...
// ************************************************************************************************************ 
  void SiStripTemplate::xsigma2(int fxpix, int lxpix, float sxthr, float xsum[BSXSIZE], float xsig2[BSXSIZE])
...
// Finally, rescale the yint value for cotalpha variation			 
           ...
           if(xsig2[i] < s50_*s50_) xsig2[i] = s50_*s50_;
